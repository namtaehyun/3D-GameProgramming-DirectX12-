...
{
private:
	CMesh * m_pAirplaneMesh[3];
public:
	CAirplanePlayer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList,
		ID3D12RootSignature *pd3dGraphicsRootSignature);
	virtual ~CAirplanePlayer();
	virtual void ChangeRedModel();
	virtual void ChangeGreenModel();
	virtual void ChangeMagentaModel();
	virtual void SetFirst();
	virtual void Targeting(CGameObject *pSelectedObject);
	virtual void TargetTo(XMFLOAT3& xmf3LookAt, XMFLOAT3& xmf3Up);
	virtual CCamera *ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed);
	virtual void OnPrepareRender();
};